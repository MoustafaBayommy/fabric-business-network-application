...
describe('Starting tests for composer-app', function() {
  ...
  it('website title should be composer-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('composer-app');
    })
  });
  ...
  it('navbar-brand should be composer-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('composer-app');
    });
  });
